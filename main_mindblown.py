...
MUSICA = [
    ...
    "https://cdn.pixabay.com/download/audio/2021/04/07/audio_c1a9a6a8c2.mp3",
    "https://cdn.pixabay.com/download/audio/2022/10/25/audio_946bc4b6b5.mp3",
]
VOLUMEN_MUSICA = 0.20  # antes 0.15 - un poco mas presente sin tapar la voz
...
- Each entry: 1-2 punchy sentences with ONE specific, verifiable, mind-blowing detail.
  Write like you're telling a friend something wild, not reading a textbook —
  use casual, punchy phrasing, vary sentence length and rhythm entry to entry
  so it doesn't feel like a monotone list. A few entries can open with a quick
  rhetorical question ("Think that's wild? ...") or a one-word reaction
  ("Insane, right? ...") to break the pattern — but don't overuse this, vary it.
...
async def _tts(texto: str, salida: str, voz: str, rate: str, pitch: str):
...
def generar_audio(texto: str, salida: str = "audio_mindblown.mp3"):
    ...
    # Un poco mas rapido y con tono ligeramente mas alto = suena mas energico
    # y entretenido que la velocidad/tono neutro por defecto. Se varia un poco
    # en cada video para que no suene siempre identico.
    rate = f"+{random.randint(8, 18)}%"
    pitch = f"+{random.randint(0, 4)}Hz"
    print(f"Voz: {voz} | rate: {rate} | pitch: {pitch}")
    asyncio.run(_tts(texto, salida, voz, rate, pitch))
    return salida
...
def armar_video(clips_por_escena, pool_generico, imagenes_ia, audio_path,
    ...
    if musica_path:
        try:
            ...
            audios.append(musica.subclip(0, duracion_video).volumex(VOLUMEN_MUSICA))
        except Exception as e:
            print(f"Aviso musica: {e}")
