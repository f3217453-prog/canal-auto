"""
Pipeline para Shorts - 6 nichos:
Horror, True Crime, World Records, Top 10, History Facts, Interesting Facts
Con imagenes IA, reintentos automaticos, sin pantalla negra, muchos clips variados
"""
...
IMPORTANT: Write everything in ENGLISH only. Return ONLY valid JSON, no markdown, no backticks:
{
  "personaje": {"nombre": "...", "descripcion": "...", "personalidad": "..."},
  "lugar": {"nombre": "...", "descripcion": "..."},
  "guion": "...(55 second narration)...",
...
Create a script about an incredible, lesser-known historical fact or event with:
1. A key historical figure involved (name, description, era)
2. The historical location or setting
3. Narrative: hook with shocking fact, historical context, surprising details, modern relevance
4. 3 vivid scene descriptions for AI image generation
...
1. A fictional scientist or expert character (name, field, description)
2. A dramatic setting where this fact is revealed or demonstrated
3. Narrative: shocking hook fact, explanation, more mind-blowing details, perspective-changing ending
4. 3 vivid scene descriptions for AI image generation
...
CONSULTAS_RESPALDO = [
    ...
    "smoke slow motion dark", "clouds timelapse",
            ...
            try:
                ...
                try:
                    ...
                    return {
                        ...
                        "guion": "Something incredible happened here that most people never knew about. The story begins in a place few have visited and fewer have escaped unchanged.",
                        "escenas": ["dramatic establishing shot", "close up mysterious detail", "wide atmospheric scene"],
...
def generar_imagenes_personaje(contenido: dict) -> list:
    ...
    prompt_personaje = (
        ...
        "cinematic lighting, dramatic, high quality"
    )
...
def armar_video(clips_pexels, imagenes_ia, audio_path, segmentos, salida="video_final.mp4"):
    ...
    # Primero las imagenes IA (3.5s cada una)
    duracion_imagen = 3.5
    ...
    # Luego clips de Pexels sin repeticion seguida
...
def main():
    ...
    tags = contenido.get("tags", [nicho, "shorts", "viral", "facts"])
