...
def transcribir(audio_path: str):
    modelo = whisper.load_model("tiny")  # más rápido que base, suficiente para subtítulos
...
def descargar_clips_largos(nicho: str, carpeta: str = "clips_largo"):
    ...
    # Un video largo necesita muchos más clips; si no llegamos a un pool
    # decente, completamos con búsquedas genéricas de respaldo
    if len(rutas) < 25:
...
def armar_video_largo(clips_rutas, audio_path, segmentos, salida="video_largo_final.mp4"):
    ...
    while tiempo_acumulado < duracion_total:
        ...
        # Clips un poco más largos que en Shorts (4-8s), pero variados
